...
Ofcourse, an arbitrary executable/script is also handled by .githooker. Just create it under `.githooks` named after the hook it shall be triggered upon and run `.githooker/enable pre-commit` - that's it!

Moreover, .githooker provides commands to simply manage git-hooks in an interactive CLI manner. The following output of `.githooker/help` shows/explains all avalaible commands:
